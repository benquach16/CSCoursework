...
Vector3 getInterpolatedNormal(Face f, Vector3 P)
{
    ...
    //jesus
    Vector3 BA = f.v2->position - f.v1->position;
    Vector3 CA = f.v3->position - f.v1->position;
    Vector3 BP = f.v2->position - P;
    Vector3 CP = f.v3->position - P;
    Vector3 AP = f.v1->position - P;
    float areaABC = f.normal.dot(BA.crossProduct(CA));
...
void drawPhong(Face f, int i, int y, int c)
{
    ...
    Vector3 viewerVector = Vector3(400,400,0) - pixel;
